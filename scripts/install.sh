...
Options:
  --dest <path>              Install base directory (skill placed in <path>/claude-relais)
  --force                    Overwrite existing install
  --model <model_id>         Orchestrator model (default: opus-4.6)
  --builder <mode>           Builder mode: cursor|claude_code (default: cursor)
...
if [[ "$BUILDER" != "cursor" && "$BUILDER" != "claude_code" ]]; then
  echo "Invalid --builder value: ${BUILDER}. Use cursor|claude_code." >&2
  exit 1
fi
...
cat > "${INSTALL_DIR}/config.local.json" <<JSON
{
...
echo "  2) Use ${INSTALL_DIR}/scripts/bootstrap-project.sh in your target repo"
