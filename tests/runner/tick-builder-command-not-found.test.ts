/**
 * Tests for BLOCKED_BUILDER_COMMAND_NOT_FOUND behavior when cursor builder mode
 * is configured but the command is not found or not executable.
 *
 * Verifies that when builder returns validationErrors including
 * BLOCKED_BUILDER_COMMAND_NOT_FOUND, runTick returns verdict=blocked with
 * code=BLOCKED_BUILDER_COMMAND_NOT_FOUND and blocked data is written.
 */
...
describe('tick: BLOCKED_BUILDER_COMMAND_NOT_FOUND for cursor builder', () => {
  ...
  beforeEach(async () => {
    ...
    // Ensure cursor config is present
    config.builder.cursor = {
      command: 'nonexistent-driver',
      args: [],
    ...
    // Mock builder to return failure with BLOCKED_BUILDER_COMMAND_NOT_FOUND
    vi.mocked(runBuilder).mockResolvedValue({
      success: false,
      result: null,
      rawResponse: `Command 'nonexistent-driver' not found or not executable. ` +
        `Please install the driver or update config.builder.cursor.command in relais.config.json.`,
      durationMs: 0,
  ...
  it('should return BLOCKED_BUILDER_COMMAND_NOT_FOUND when command not found', async () => {
    const report = await runTick(config);

    expect(report.verdict).toBe('blocked');
    expect(report.code).toBe('BLOCKED_BUILDER_COMMAND_NOT_FOUND');
  ...
  it('should write BLOCKED.json with actionable remediation', async () => {
    ...
    expect(blockedData.code).toBe('BLOCKED_BUILDER_COMMAND_NOT_FOUND');
    expect(blockedData.reason).toContain('not found or not executable');
    expect(blockedData.reason).toContain('install the driver');
  });
  ...
  it('should include actionable warning in report', async () => {
    ...
    const warning = report.budgets.warnings.find((w) =>
      w.includes('not found or not executable')
    );
    expect(warning).toBeDefined();
    expect(warning).toContain('install the driver');
  });
  ...
  it('should persist builder failure artifacts for debugging', async () => {
    ...
    expect(callArgs[3].details.validationErrors).toContain('BLOCKED_BUILDER_COMMAND_NOT_FOUND');
  });
});
