 ...
 * 
 * Verify that question task kind with edits results in
...
describe('F003: question_no_side_effects', () => {
  it('should detect STOP_QUESTION_SIDE_EFFECTS when question task has diff', () => {
    const task = createMockTask('question');
    const hasDiff = true; // Simulate that builder edited files

    const code = determineReportCode(task.task_kind, hasDiff);

    expect(code).toBe('STOP_QUESTION_SIDE_EFFECTS');
  });

  it('should not detect side effects when question task has no diff', () => {
    const task = createMockTask('question');
  ...
  it('should validate scope even for question tasks', () => {
    ...
    // But the judge phase would detect STOP_QUESTION_SIDE_EFFECTS due to diff existing
